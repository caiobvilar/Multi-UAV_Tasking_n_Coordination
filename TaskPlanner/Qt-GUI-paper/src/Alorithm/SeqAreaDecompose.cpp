                                                            ...
                                                            std::unordered_map<int, std::vector<int>> &sub_triangles, double area)
{
    ...
    for (auto &sep_line : tri_sep_lines)
    {
        ...
        std::transform(norm_tri.begin(), norm_tri.end(), norm_tri.begin(), [&](double val)
                       { return val / sum; }); // normalization
        ...
        for (auto &l : sub_lines)
        {
            ...
            else if (!tasks.empty() && static_cast<std::size_t>(select) < batteries_.size())
            {
